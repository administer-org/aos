...
async def submit_feedback(req: Request):
    is_blocked = db.get(req.headers.get("Roblox-Id", 0), db.ABUSE_LOGS) # using abuse_logs bc i can lmao
    ...
    try:
        if ( # this is kinda overkill, but i don't want abuse
            len(json["Comment"]) >= 700 or
            len(json["Administer"]) >= 300 or
            len(json["What"] + json["Where"] + json["Priority"]) >= 150 or
            db.get(req.headers["Roblox-Id"], db.PLACES) is None
        ):
            return JSONResponse({
                "title": "Bad input", 
